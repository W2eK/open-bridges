...
const BridgeItem = props => {
  const { name, moveable, schedule, time } = props;
  ...
  return (
    <li className={classNames.bridgeItem}>
      <h4 className={current % 2 ? classNames.bridgeItem__inactive : null}>{name}</h4>
      <BridgeProgress segments={segments} current={current} time={scaler(parseTime(time) - 1)}/>
      <BridgeTime segments={segments} schedule={schedule} moveable={moveable} current={current}/>
    </li>
  );
};
